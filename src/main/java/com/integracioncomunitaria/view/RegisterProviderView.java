...
public class RegisterProviderView extends JFrame {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        ResultDataBase userResult = userController.registerUser(user, "proveedor");

        if (userResult.getSuccess()) {
            int userId = (int) userResult.getObject();

            // Registrar el proveedor
            Provider provider = new Provider(name, userId);
            ProviderController providerController = new ProviderController();
            ResultDataBase providerResult = providerController.registerProvider(provider);

            if (providerResult.getSuccess()) {
                // Registrar el perfil del usuario
                UserProfileController profileController = new UserProfileController();
                ResultDataBase profileResult = profileController.registerUserProfile(userId, email, "proveedor");

                if (profileResult.getSuccess()) {
                    JOptionPane.showMessageDialog(null, "Proveedor registrado correctamente.");
                    new ProfileSetupView(userId).setVisible(true);
                    dispose();
                } else {
                    JOptionPane.showMessageDialog(null, profileResult.getMessage());
                }
            } else {
                JOptionPane.showMessageDialog(null, providerResult.getMessage());
            }
        } else {
